...
Painless data visualizations from git history showing a repository's development progression over time.
...
This container is 100% headless; it does this by leveraging [Xvfb][xvfb] combined with the [Mesa 3D Gallium llvmpipe Driver][mesa]. Unlike other Docker containers with Gource, this container does not consume hundreds of gigabytes of disk space, nor does it require an actual GPU to run. The process runs the Gource simulation concurrently with the FFmpeg encoding process using a set of named pipes. There is a slight trade-off in performance, but this makes it very easy to run in any environment such as AWS without the need to provision large amounts of storage or run any cleanup.
...
| -------------------------- | -------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| `GIT_URL`                  | `<docker repo on GH>`            | URL of the git repository to be cloned and analyzed for visualization.                                      |
| `LOGO_URL`                 | `<docker logo>`                  | URL of the logo to be overlaid in the lower right-hand corner of the video.                                 |
...
| `GOURCE_TITLE`             | `Software Development`           | Title to be displayed in the lower left-hand corner of the video.                                           |
...
| `GOURCE_AUTO_SKIP_SECONDS` | `0.5`                            | Skip to the next entry if nothing happens for a number of seconds.                                          |
